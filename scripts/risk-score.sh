...
#                          плана. Диффа ещё нет, поэтому размер по строкам и
#                          ключевые слова не считаются, и режим это печатает;
#                          размер по числу файлов считается по files[].
#   diff <base> <head>   — после реализации: имена файлов, добавленные и
#                          удалённые строки, numstat. Словарная половина
#                          читает обе: снятие защиты — удалённая строка с
#                          RequireAuthorization — такое же событие зоны, как
#                          появление анонимности. Не ловится файл,
#                          переименованный из keyword_paths наружу, если
#                          других путей из области в диффе нет: область
#                          сверяется с --name-only, а там у переименования
#                          только новый путь, и строки не читаются вовсе.
#                          При переименовании без правки содержимого строк
#                          нет и тогда, когда они читаются.
...
# строки кода, и у готового диффа. У размера симметрия половинная: число
# файлов есть у обоих входов, число строк — только у диффа.
#
# КАК ЗАПУСКАТЬ
#
...
# ли вообще дорогой дифф с добавленными и удалёнными строками.
KW_SCOPE_UNION=''
...
FACT_CHANGED='' # добавленные и удалённые строки: «путь<TAB>знак<TAB>содержимое», знак + или -
FACT_LINES=0
FACT_FILES=0
FACT_KIND=''    # plan | diff
...
collect_diff_facts() {
    ...
    # Строки диффа нужны только словарной половине сигналов. Если ни один
    ...
    if [ "$scope_needed" -eq 1 ]; then
        # Тот же отказ, что выше, и та же цена: пустые строки диффа — это
        # молчание словарной половины на диффе, который не прочитан.
        #
        # Выдаются и добавленные, и удалённые строки, со знаком. Путь строки
        # «+» — из заголовка «+++ b/», строки «-» — из «--- a/»: у удалённого
        # файла «+++ /dev/null», и без старого пути его строки пропали бы.
        #
        # Заголовки разбираются только в зоне заголовка файла — от «diff --git»
        # до первого «@@». Шаблон на любой строке принимал содержимое за
        # заголовок: в -U0 удалённая «-- …» выглядит как «--- …», добавленная
        # «++ …» — как «+++ …», и строка пропадала вместе с путём следующих.
        if ! FACT_CHANGED="$(git -C "$REPO" -c core.quotepath=false diff -U0 --no-color --no-ext-diff "$base" "$head" | awk '
            ...
            # сдвинула бы поле знака.
            function header_path(line, prefix,    p) {
                p = substr(line, 5)
                sub(/\t$/, "", p)
            ...
            /^diff --git / { in_header = 1; old_path = ""; new_path = ""; next }
            in_header && /^--- / { old_path = header_path($0, "a"); next }
            in_header && /^\+\+\+ / { new_path = header_path($0, "b"); next }
            /^@@/ { in_header = 0; next }
            in_header { next }
            /^\+/ { if (new_path != "") print new_path "\t+\t" substr($0, 2); next }
            /^-/ { if (old_path != "") print old_path "\t-\t" substr($0, 2); next }
        ')"; then
            die "Строки диффа не прочитаны: git diff -U0 $base $head в $REPO — счёт не состоялся."
        fi
    fi
...
evaluate() {
    ...
    local paths_re reasons flag path hit hits word line rest sign text
    local -a flag_parts=()
    local -A seen_pairs=()
    ...
    for ((i = 0; i < SIGNAL_COUNT; i++)); do
        ...
        if [ "$kind" = 'size' ]; then
            # У диффа обе половины размера — строки и файлы. У плана диффа нет
            # только наполовину: строк ещё нет, а число файлов он объявляет в
            # files[], и порог по файлам тот же. Поэтому в режиме plan работает
            # ровно половина по файлам, и причина говорит, что строки не мерились.
            if [ "$FACT_KIND" = 'diff' ] \
            ...
            elif [ "$FACT_KIND" = 'plan' ] && [ "$THRESHOLD_FILES" != 'нет' ]; then
                if [ "$FACT_FILES" -gt "$THRESHOLD_FILES" ]; then
                    reasons+="      размер плана: файлов: $FACT_FILES (порог $THRESHOLD_FILES); строки не измерялись — диффа ещё нет"$'\n'
                fi
            fi
            ...
            #
            # Условия на FACT_KIND здесь нет намеренно: подсистемы считаются из
            # путей, а пути есть у обоих входов, и ширина одинакова в обоих
            # режимах. У ветки размера условие есть — в режиме plan у неё
            # работает только половина по числу файлов.
            ...
            # Словарная половина — только внутри keyword_paths, по добавленным
            # и удалённым строкам одинаково: слова и область те же. В режиме
            # plan строк нет вовсе.
            #
            # Область применяется к пути, а не к строке: за альтернативой стоят
            # табуляция, знак и ещё одна табуляция, которыми путь отделён от
            # содержимого. Путей с табуляцией внутри git не отдаёт.
            if [ -n "${SIG_KW_RE[$i]}" ] && [ -n "$FACT_CHANGED" ] \
                && [ -n "${SIG_KWSCOPE_RE[$i]}" ]; then
                hits="$(printf '%s\n' "$FACT_CHANGED" \
                    | grep -iE -- "^(${SIG_KWSCOPE_RE[$i]})"$'\t''[+-]'$'\t'".*(${SIG_KW_RE[$i]})")"
                if [ -n "$hits" ]; then
                    # Дубли отсекаются по паре «путь, знак»: снятая защита и
                    # добавленное слово в одном файле — две разные причины.
                    seen_pairs=()
                    shown=0
                    while IFS= read -r line; do
                        [ -z "$line" ] && continue
                        ...
                        sign="${rest%%$'\t'*}"
                        text="${rest#*$'\t'}"
                        [ -n "${seen_pairs[$path$'\t'$sign]+есть}" ] && continue
                        seen_pairs["$path"$'\t'"$sign"]=1
                        shown=$((shown + 1))
                        # Слово ищется только в содержимом: путь с тем же
                        # словом назвал бы совпадение, которого в строке нет.
                        word="$(printf '%s' "$text" \
                            | grep -m1 -ioE -- "${SIG_KW_RE[$i]}")"
                        if [ "$sign" = '-' ]; then
                            reasons+="      слово в удалённой строке: $word — $path"$'\n'
                        else
...
print_verdict() {
    ...
    if [ "$FACT_KIND" = 'plan' ]; then
        printf 'Не измерялось в режиме plan: размер диффа по строкам и ключевые '
        printf 'слова по добавленным и удалённым строкам — диффа ещё нет. Размер по числу '
        printf 'файлов считается по files[] плана.\n'
    fi

    # Строка о незаданном пороге размера печатается в обоих режимах, но условия
    # у них разные: диффу нужны оба порога, плану — только порог файлов, потому
    # что строк у него нет. Без строки в режиме plan незаданный порог файлов
    # был бы молчанием, неотличимым от «размер в пределах порога».
    if { [ "$FACT_KIND" = 'diff' ] \
            && { [ "$THRESHOLD_LINES" = 'нет' ] || [ "$THRESHOLD_FILES" = 'нет' ]; }; } \
        || { [ "$FACT_KIND" = 'plan' ] && [ "$THRESHOLD_FILES" = 'нет' ]; }; then
        printf 'Порог размера в конфиге не задан: сигнал размера не считался. '
        printf 'Его печатает первый проход подкомандой history.\n'
    fi

    # Условия FACT_KIND здесь нет вовсе — в отличие от строки о пороге размера,
    # у которой условия по режимам разные: ширина считается одинаково в обоих.
