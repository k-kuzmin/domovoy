...
#      меткой нет.
#  11. Снятие ограничений схемы в миграции вызовом API EF: индекса, внешнего,
...
# (b.HasKey), под имя AddPrimaryKey не подпадает. Снятие индексов и ключей
# (DropIndex, DropForeignKey, DropPrimaryKey, DropUniqueConstraint) ни одному
# из трёх признаков не отвечает и ловится своей проверкой 11.
#
...
# есть, — поэтому сценарий переименования держит фильтр, а не -M.
#
# Шаблон каталога совпадёт и с каталогом вроде TestResultFormatter/ — это то
# же множество, что скрывает .gitignore, и файл туда без git add -f не
# попадает.
#
...
# EXISTS и CREATE INDEX … IF NOT EXISTS корректны и нарушением не считаются.
#
# Сравнение без учёта регистра: шаблоны в нижнем регистре, строка приводится
# к нему же. Слова внутри правила
...
#     здесь тоже не ловятся — их ловит проверка 11, а сырые DROP INDEX и DROP
#     CONSTRAINT ловит правило «DROP без IF EXISTS»;
...
# ------------------------------------------------------------------
# Проверка 11. Снятие ограничений схемы в миграции
#
# Проверка своя, а не расширение списка проверки 4: у той записан критерий
# допуска — потеря данных, переименование или смена типа, перестройка живой
# таблицы под блокировкой, — и снятие индекса или ключа ему не отвечает.
# Решение владельца в #125: критерий проверки 4 не размывается.
#
# Ловится вызов потому, что схема и код деплоятся не атомарно: снятый индекс
# замедляет запросы работающей версии, снятый внешний, первичный или
# уникальный ключ снимает гарантию целостности, на которую она опирается.
# Сырые DROP INDEX и DROP CONSTRAINT ловит проверка 9 — здесь только имена API
# EF, с учётом регистра, как у проверки 4.
#
# Граница — добавленные строки файла, а не метод, та же, что у проверки 4:
# Down() обычной миграции, которая создаёт индекс или внешний ключ, откатывает
# их через DropIndex и DropForeignKey, и такая миграция тоже требует метки.
# Граница пересматривается после первой настоящей миграции вместе с проверкой
# порядка версий (#125). Харнесс синтетический, как у проверки 9: миграций в
# дереве ноль. Шаблон ищет голое имя метода, поэтому перенос вызова на
# следующую строку после migrationBuilder ему не мешает. Обход — метка
# agent/allow-destructive-migration, та же, что у проверок 4 и 9.
# ------------------------------------------------------------------
CONSTRAINT_DROP_RE='(DropIndex|DropForeignKey|DropPrimaryKey|DropUniqueConstraint)'

CONSTRAINT_DROP_HIT="$WORK/constraint-drop.tsv"
select_lines '+' "$CONSTRAINT_DROP_RE" "$MIGRATION_FILE_RE" > "$CONSTRAINT_DROP_HIT"

if [ -s "$CONSTRAINT_DROP_HIT" ] && [ "$ALLOW_DESTRUCTIVE" -eq 0 ]; then
    section 'Проверка 11: снятие ограничений схемы в миграции'
    while IFS=$'\t' read -r file line text; do
        [ -n "$file" ] || continue
        annotate "$file" "$line" \
            "Снятие индекса или ключа в миграции: $file. Схема и код деплоятся не атомарно: снятый индекс замедляет запросы работающей версии, снятый внешний, первичный или уникальный ключ снимает гарантию целостности, на которую она опирается. Откат обычной миграции, создающей индекс или ключ, тоже снимает его — такой Down() требует той же метки. Нужна метка agent/allow-destructive-migration на PR — её ставит человек, убедившись, что ограничение не нужно работающей версии."
        quote_line "$file" "$line" "$text"
    done < "$CONSTRAINT_DROP_HIT"
elif [ -s "$CONSTRAINT_DROP_HIT" ]; then
    section 'Проверка 11: снятие ограничений схемы разрешено меткой agent/allow-destructive-migration'
    while IFS=$'\t' read -r file line text; do
        [ -n "$file" ] || continue
        printf '    разрешено: %s:%s\n' "$file" "$line"
    done < "$CONSTRAINT_DROP_HIT"
fi
