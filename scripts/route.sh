...
#
# Ключи: --config <файл> (другая политика) — у обеих подкоманд. --repo <каталог>
# (другое дерево — так работают сценарии харнесса scripts/route.test.sh) —
# только у check: подкоманда level печатает политику и в дерево не смотрит,
# поэтому принятый там ключ выглядел бы поддерживаемым, ничего не меняя.
#
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --repo)
            ...
            # Ключ есть только у check — так он и напечатан в usage. У level
            # значение ключа никуда не идёт: дерево она не читает. Молча
            # принятый ключ обещает поддержку, которой нет, поэтому отказ.
            [ "$MODE" = 'check' ] || {
                usage >&2
                die 'Ключ --repo есть только у подкоманды check: level печатает политику и дерева не читает.'
            }
            REPO="$2"
            shift 2
            ;;
...
# Путь для аннотации: от корня дерева, а не абсолютный. GitHub сопоставляет
# аннотацию со строкой диффа только по относительному пути, абсолютный он не
# привязывает ни к одному файлу — нарушение остаётся в логе и исчезает оттуда,
# где на него смотрят. Пути потребителей приходят из политики уже
# относительными, а $CONFIG собран от корня, и разъезжались ровно те пять
# вызовов, где предмет нарушения — сама политика.
#
# Путь вне дерева остаётся как есть: у временной политики из --config пути от
# корня дерева не существует, и выдумывать его хуже, чем напечатать честно.
relative_to_tree() {
    local file="$1" base
    for base in "$REPO" "$ROOT"; do
        case "$file" in
            "$base"/*)
                printf '%s' "${file#"$base"/}"
                return 0
...
report() {
    ...
    # Нормализация пути стоит здесь, а не в вызовах: в вызове её однажды
    # забудут, и следующая аннотация повторит ту же ошибку.
    printf '::error file=%s::%s\n' "$(relative_to_tree "$1")" "$2"
