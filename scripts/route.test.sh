...
# ------------------------------------------------------------------
begin_case 'Ключ --repo вне подкоманды check отклоняется, а не проглатывается'
# Пара к новой ветке отказа движка. У подкоманды level дерева нет, значение
# ключа никуда не идёт, и принятый молча ключ обещал бы поддержку, которой нет:
# usage печатает --repo только у check.
run_route level low --repo "$SANDBOX"
expect_status 2
expect_output '--repo'
expect_output 'Использование:'
expect_no_output 'Уровень: low'

# Положительная половина: у check тот же ключ работает и дерево доезжает до
# вывода. Без неё сценарий доказывал бы лишь то, что ключ сломан вообще.
if new_tree; then
    run_route check --repo "$TREE"
    expect_status 0
    expect_output "Дерево: $TREE"
fi
end_case
...
# ------------------------------------------------------------------
begin_case 'Значение с переводом строки внутри роняет разбор, а не разъезжается молча'
# Пара к инварианту самого разбора: записей прочитано столько же, сколько
# позиций в политике. Перевод строки внутри значения — законная строка JSON, и
# проверку формы она проходит: та смотрит тип и непустоту. Разрезанная надвое
# запись сдвинула бы поля позиции, и маршрут стал бы мусором молча. Ни одна
# мутация выше такого значения не подставляет: они портят отсутствующие, пустые
# и нестроковые поля, то есть ветка отказа инварианта была недостижима.
POS_IN_POLICY="$(pjq '.positions | length')"
MUT_NEWLINE="$(mutate_policy \
    '.positions[0].levels.low.value = "значение с переводом строки\nи продолжением на второй строке"')"
if ! jq -e '.positions[0].levels.low.value | contains("\n")' "$MUT_NEWLINE" >/dev/null; then
    fail_case 'мутация не состоялась: перевода строки в значении нет, разрезать разбору нечего'
else
    run_route level low --config "$MUT_NEWLINE"
    expect_status 2
    expect_output 'Разбор политики разъехался'
    # Число прочитанных записей и есть высказывание инварианта: запись
    # разрезало, значит прочитано на одну больше, чем позиций в политике.
    # Без этого ожидания сценарий зеленел бы и на неразрезанной записи.
    expect_output "прочитано $((POS_IN_POLICY + 1)) записей"
    expect_output "в политике $POS_IN_POLICY позиций"
    expect_no_output 'Уровень: low'

    # Сверка на такой политике не зеленеет: разбор отказывает до неё.
    run_route check --config "$MUT_NEWLINE"
    expect_status 2
    expect_no_output 'Маршрут сходится'
fi
end_case
...
# ------------------------------------------------------------------
begin_case 'Аннотация нарушения указывает путь от корня дерева, а не абсолютный'
# GitHub привязывает аннотацию к строке диффа только по относительному пути:
# абсолютный не сопоставляется ни с одним файлом, и нарушение исчезает оттуда,
# где на него смотрят, оставаясь в логе. Проверяется на политике внутри
# временного дерева — только у такой есть путь от его корня. Нарушение берётся
# то, чей предмет — сама политика: пустой список «не сокращается никогда»
# проверку формы проходит (она смотрит тип и элементы), а сверку роняет.
if new_tree; then
    IN_TREE_POLICY="$TREE/$CONFIG_REL"
    mkdir -p "$(dirname "$IN_TREE_POLICY")"
    MUT_NEVER_EMPTY="$(mutate_policy '.never_reduced = []')"
    cat "$MUT_NEVER_EMPTY" > "$IN_TREE_POLICY"
    run_route check --repo "$TREE" --config "$IN_TREE_POLICY"
    expect_status 1
    expect_output 'список never_reduced пуст'
    expect_output "::error file=$CONFIG_REL::"
    # Ни абсолютного пути временного дерева, ни любого другого: аннотация с
    # ведущей косой чертой — это и есть непривязанная аннотация.
    expect_no_output "::error file=$TREE"
    expect_no_output '::error file=/'
fi
end_case
...
# И сверка обвязки не считает ни движок, ни харнесс неподключёнными. Предмет
# здесь — четыре ожидания ниже, а не исход сверки обвязки целиком: нарушение в
# любом несвязанном скрипте покраснило бы этот сценарий под чужим именем. От
# прогона нужно ровно одно — чтобы он состоялся: на коде 2 (сверка не
# запустилась) вывод пуст, и четыре ожидания прошли бы вхолостую.
OUTPUT="$(bash "$SCRIPT_DIR/wiring.sh" "$ROOT" 2>&1)"
STATUS=$?
if [ "$STATUS" -eq 2 ]; then
    fail_case 'сверка обвязки не запустилась (код 2): подключение прогонов проверять нечем'
fi
expect_output 'Скриптов:'
