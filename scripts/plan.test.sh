...
# Вывод до начала блока. Обрезка по первому вхождению: блок один и последний,
# и «до первого» отличается от «до последнего» только когда что-то сломалось —
# это ловит сценарий «Обрезка вывода по началу блока проверяется на себе».
prose_only() {
    printf '%s' "${OUTPUT%%<details>*}"
}
