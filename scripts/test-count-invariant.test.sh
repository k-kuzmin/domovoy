...
# ------------------------------------------------------------------
# Сценарий 11. В отчёте нет элемента <Counters> вовсе.
#
# Отличается от сценария 7: там элемент есть, но без нужного атрибута. Здесь
# сводки нет совсем — так выглядела бы смена формата trx, а не одного атрибута.
# Ветка обязана быть красной: без счётчика инвариант сравнивал бы не то, что
# думает.
# ------------------------------------------------------------------
begin_case 'в trx нет элемента <Counters> — код 1'
new_case
cat > "$work/TestResults/tests.trx" <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<TestRun id="00000000-0000-0000-0000-000000000000">
  <ResultSummary outcome="Completed">
    <Output>
      <StdOut>Passed! - Failed: 0, Passed: 42, Skipped: 0</StdOut>
    </Output>
  </ResultSummary>
</TestRun>
EOF
...
run_invariant "$work"
expect_status 1
expect_output 'нет элемента <Counters>'
end_case

# ------------------------------------------------------------------
# Сценарий 12. Атрибут, кончающийся на executed, стоит раньше executed.
#
# Сценарий существует, чтобы пробел в шаблоне [[:space:]]executed=" нельзя
# было снять молча: уберите пробел — и здесь прочитается 2 вместо 40, отчёт
# «стало меньше», код 1.
#
# Настоящий trx пишет notExecuted с заглавной E, а grep -oE регистр
# различает — на настоящем отчёте ложного совпадения нет ни с пробелом, ни
# без него, и сценарий 9 его не различает. Фикстура воспроизводит ровно ту
# гипотетическую смену регистра, которой пробел и обоснован в шапке проверки:
# порядок атрибутов в trx не оговорён, поэтому «раньше» тоже часть условия.
# ------------------------------------------------------------------
begin_case 'атрибут notexecuted перед executed не подменяет число'
new_case
cat > "$work/TestResults/tests.trx" <<'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<TestRun id="00000000-0000-0000-0000-000000000000">
  <ResultSummary outcome="Completed">
    <Counters total="42" notexecuted="2" executed="40" passed="40" failed="0" />
  </ResultSummary>
</TestRun>
EOF
write_baseline "$work" 40
run_invariant "$work"
expect_status 0
expect_output 'Выполнено тестовых случаев: 40'
expect_output 'сходится'
end_case
