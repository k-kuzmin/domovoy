...
# То же, что require, но шаблоном: нужно, когда искомое — не строка целиком, а
# два условия на одной строке (префикс токена и то, что он покрывает).
require_regex() {
    if ! printf '%s\n' "$1" | grep -qE -- "$2"; then
        printf 'нет: %s — %s\n' "$3" "$2"
        PROBLEMS=$((PROBLEMS + 1))
...
check_publish_token() {
    PROBLEMS=0
    local block prefixed
    block="$(step_block "$1" 'Публикация плана')"
    if require_block "$block" 'шаг «Публикация плана»'; then
        # Токен шага — штатный. PAT нужен двум командам, а не всему окружению:
        # в том же окружении работают jq над выводом модели и
        # scripts/plan.sh, и радиус PAT держался бы их свойствами, а не
        # конструкцией шага.
        require "$block" 'GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}' 'штатный токен на уровне шага'
        require "$block" 'AGENT_TOKEN: ${{ secrets.AGENT_TOKEN }}' 'PAT под собственным именем'

        # Команд под PAT ровно две — счёт, а не только поиск. Поиск ловит
        # потерянный префикс, но не расползание PAT на третью команду, а
        # предмет проверки здесь — именно радиус.
        prefixed="$(printf '%s\n' "$block" | grep -cF 'GH_TOKEN="$AGENT_TOKEN"')"
        if [ "$prefixed" -ne 2 ]; then
            printf 'команд под PAT в шаге публикации: %s, ожидалось 2\n' "$prefixed"
            PROBLEMS=$((PROBLEMS + 1))
        fi

        # Комментарий от штатного токена приходит от бота, чей
        # authorAssociation не равен OWNER, — выборка предмета ревью не нашла
        # бы плана ни на одной задаче. Метка от штатного токена не породила бы
        # события, и круг не стартовал бы вовсе.
        require "$block" 'GH_TOKEN="$AGENT_TOKEN" gh issue comment' \
            'комментарий с планом под PAT'
        require_regex "$block" 'GH_TOKEN="\$AGENT_TOKEN" gh issue edit .*--add-label .plan/proposed.' \
            'метка plan/proposed под PAT'
...
# Предмет ревью выбирает шаг, а не модель. Проверяется выборка и ветка отказа,
# а не фраза промпта: промпт — текст, он правится свободно, и мера, стоящая
# только на нём, снимается одной правкой без единого красного сценария.
check_prompt() {
    PROBLEMS=0
    local body block
    body="$(job_body "$1" plan-correctness)"
    if require_block "$body" 'тело job plan-correctness'; then
        # Гейт job доказывает авторство ЗАДАЧИ; комментарий в публичной задаче
        # вправе оставить кто угодно, и подделку по форме плана отсекает
        # только эта выборка.
        require "$body" 'select(.authorAssociation == "OWNER")' \
            'выборка комментария владельца в шаге'
        require "$body" 'last | .body' 'из комментариев владельца берётся последний'
        # Комментария владельца нет — падает шаг. Вердикт об этом от модели
        # был бы платным выводом, известным до вызова.
        require "$body" '[ ! -s plan-comment.md ]' \
            'отказ шага при отсутствии комментария владельца'
    fi
    block="$(step_block "$1" 'Проверка плана')"
    if require_block "$block" 'шаг вызова модели'; then
        require "$block" 'plan-comment.md' 'предмет ревью назван модели файлом'
        # Без инструмента мера мертва в другую сторону: задачу — критерии
        # приёмки, на выполнимость которых план и проверяется, — модель берёт
        # этим вызовом.
        require "$block" 'Bash(gh issue view' 'просмотр задачи в наборе инструментов'
    fi
    [ "$PROBLEMS" -eq 0 ]
}
...
# ------------------------------------------------------------------
# Сценарий 10. Токен публикации плана: PAT на двух командах, шаг штатный.
#
# Правка, переведшая на PAT одну лишь постановку метки, оставила бы автором
# комментария бота: круг стартовал бы и падал на отсутствии комментария
# владельца — на каждой задаче. Обратная крайность, PAT на всём шаге, даёт
# радиус шире нужного: в том же окружении работают jq и scripts/plan.sh.
# ------------------------------------------------------------------
begin_case 'Токен публикации плана: PAT на двух командах, шаг под штатным токеном'
...
# ------------------------------------------------------------------
# Сценарий 12. Потерянный префикс PAT у комментария с планом ловится.
#
# Секрет при этом остаётся в окружении шага, и глазами правка выглядит
# безобидной: комментарий публикуется, но уже от бота.
# ------------------------------------------------------------------
begin_case 'Токен публикации плана: потерянный префикс у комментария ловится'
run_check check_publish_token "$(copy_without "$PLAN_WF" 'GH_TOKEN="$AGENT_TOKEN" gh issue comment')"
expect_status 1
end_case

# ------------------------------------------------------------------
# Сценарий 13. Вердикт changes_requested: три операции с метками.
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Сценарий 14. Пропажа снятия agent/ready ловится.
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Сценарий 15. Предмет ревью выбирает шаг: выборка, отказ, инструмент.
# ------------------------------------------------------------------
begin_case 'Предмет ревью: выборка комментария владельца стоит в шаге, а не в промпте'
run_check check_prompt "$REVIEW_WF"
expect_status 0
end_case

# ------------------------------------------------------------------
# Сценарий 16. Пропажа выборки ловится.
#
# Без неё предметом ревью снова становится то, что модель найдёт сама, —
# включая комментарий постороннего, написанный по форме плана.
# ------------------------------------------------------------------
begin_case 'Предмет ревью: пропажа выборки комментария владельца ловится'
run_check check_prompt "$(copy_without "$REVIEW_WF" 'select(.authorAssociation == "OWNER")')"
expect_status 1
end_case
