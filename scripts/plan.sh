...
#
# ЧТО ПЕЧАТАЕТ РЕНДЕР СВЕРХ MARKDOWN
#
#   Последним в выводе render идёт свёрнутый блок <details> с машиночитаемой
#   формой — содержимым файла плана без переформатирования: через jq оно не
#   идёт. Побайтового равенства с файлом размещение не обещает: хвостовые
#   переводы строки сводятся к одному, поэтому равенство формы и входа
#   проверяется канонизацией jq -S. Так форма переживает публикацию
...
#   дошедший до конца, от половины тела с кодом 2. «Отказ без тела»
#   размещение не обещает: die пишет в stderr и уже выведенного не отменяет.
#
...
do_render() {
    ...
    # размещение обещает. «Отказ без тела» оно не обещает — die пишет в
    # stderr и уже выведенного не отменяет.
    #
    # Зачем форма переживает публикацию — docs/rules/plan.md, раздел
    # «Форма — машиночитаемая».
    printf '<details>\n'
    printf '<summary>Машиночитаемая форма плана</summary>\n\n'
    printf '```json\n'
    # Содержимое плана не переформатируется — через jq оно не идёт: фикстура
    # переносится из блока копией, и переформатирование сделало бы копию
    # неотличимой от восстановления. Побайтового равенства с файлом это не
    # обещает: подстановка сводит хвостовые переводы строки к одному — ровно
    # чтобы закрывающий забор не приклеился к последней строке плана.
    printf '%s\n' "$(cat -- "$PLAN")"
    printf '```\n\n'
    printf '</details>\n'
    exit 0
