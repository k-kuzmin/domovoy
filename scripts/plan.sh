...
#   формой — тем же файлом плана байт в байт. Так форма переживает публикацию
...
do_render() {
    ...
    printf '```json\n'
    # Байт в байт, а не через jq: фикстура переносится из блока копией, и
    # переформатирование сделало бы копию неотличимой от восстановления.
    # Подстановка нормализует только хвостовые переводы строки — ровно чтобы
    # закрывающий забор не приклеился к последней строке плана.
    printf '%s\n' "$(cat -- "$PLAN")"
    printf '```\n\n'
    printf '</details>\n'
    exit 0
