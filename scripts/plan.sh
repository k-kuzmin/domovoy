...
#   пустые строки, поля, не описанные схемой. Своего списка полей у скрипта
#   нет: копия разъезжается со схемой молча.
#
...
#     4. пункт контракта приёмки не покрыт ни одним тестом;
...
#   несуществующий путь или на строку за концом файла; новая зависимость без
