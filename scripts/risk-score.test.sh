...
if put_from_tree 'pipeline/risk.json' 'pipeline/risk.json' \
    ...
    # конфигурации приложения. С появлением политики маршрута pipeline/** стал
    # ещё и защищённым путём — гейт целостности требует на такой правке метку
    # человека (запись 0020), — и сигнал этим не отменяется: метка означает
    # «человек посмотрел», а уровень отвечает на другой вопрос.
