...
Ещё один инструмент нужен, только если задачу ведёт локальный агент: **`jq`**.
На нём держится граница команд шага — `scripts/step-bash-allow.sh` проверяет,
что команда есть в списке разрешённого для этого шага. Без `jq` хук отказывает
во всех вызовах Bash у субагентов: пропустить непроверенную команду он не
вправе. Обычной разработке руками `jq` не нужен.
