...
секреты не заполнены, `/health` покажет для них состояние
«не сконфигурировано», а не упадёт.
...
Ожидаемый ответ — `Healthy` или `Degraded` с перечнем компонентов: без ключей
LLM и токена Home Assistant состояние деградированное, но сервис поднят.

Скопированные `config/*.yaml` и `.env` закрыты `.gitignore` — заполнять их
реальными значениями безопасно.
