...
### Приложение Claude на репозитории — обязательно

Это единая точка отказа всего цикла, и она не про секрет.

Агентские процессы намеренно **не передают** действию `github_token`. Причина
в правиле GitHub: событие, вызванное штатным `GITHUB_TOKEN`, не запускает
другие рабочие процессы. Триаж поставил бы `agent/ready` — и `agent-plan.yml`
на это не проснулся бы. Реализация запушила бы ветку — и быстрый CI не
запустился бы. Цепочка обрывается молча, без единой ошибки в логах.

Без `github_token` действие меняет OIDC-токен запуска на токен приложения
GitHub, и события от него нормальные. Цена: **если приложение Claude на
репозиторий не установлено, все пять процессов падают** на шаге с моделью.

Проверить установку заранее нельзя обычным токеном `gh` — только открыть
настройки репозитория, раздел GitHub Apps.

**Заодно проверьте логин приложения.** В `agent-fix.yml` он встречается в
гейте доступа и в списке `allowed_bots`, в `agent-plan.yml` — в `allowed_bots`.
Проставлено `claude[bot]`; если приложение представляется иначе, неверны сразу
все три места, и цикл встанет — опять же молча. Логин виден в авторе первого
же комментария, оставленного агентом.

Альтернатива, если ставить приложение не хочется: классический PAT в секрете и
передача его как `github_token`. Это ещё один долгоживущий секрет с правом
записи в репозиторий — потому и не выбрано по умолчанию.
...
- [ ] Установить приложение Claude на репозиторий и сверить логин бота
      (см. «Приложение Claude на репозитории»). Без этого цикл не работает.
