...
| Определения `.claude/agents/*.md` | только локальный режим | нет |
...
Определения шагов лежат в `.claude/agents/step-<шаг>.md`. Точки вмешательства
человека те же: апрув плана и мердж.

В том же каталоге живёт второй класс — **служебные агенты**, определения без
префикса `step-`. Шага работы за ними нет: они отвечают оркестратору на один
вопрос и ничего не публикуют, поэтому ни строки в таблице шагов, ни промпта в
цикле у них не бывает. Первый из них — `issue-scout`, выжимка по трекеру перед
заведением задачи. Класс выводится из имени файла, и `scripts/rules-sync.sh`
проверяет форму обоих; чем требования к классам различаются — запись
[0021](decisions/0021-service-agents-alongside-steps.md).
...
flowchart TD
    ...
    plan -->|"валидатор пропустил, уровень выше low: plan/proposed"| planreview["Круг ревью плана<br/>выполнимость, не замысел"]
    plan -->|"валидатор пропустил, уровень low: круга нет"| planok
    planreview -->|замечания| clar
    planreview -->|фактов не разошлось| planok["План ждёт человека"]
    plan -->|валидатор не пропустил| clar
    planok -->|владелец не согласен| clar
    planok -->|владелец согласен| approved["plan/approved"]
...
Одна стрелка схемы обещает чуть больше, чем цикл делает: узел `clar` описан
как «статус Blocked», но **от вердикта планового круга карточка на доске не
переедет**. Свои метки круг ставит штатным `GITHUB_TOKEN`, а события от него
не создают запусков — `status-sync.yml` не проснётся. Метка при этом стоит, и
человек видит состояние в задаче; двигать карточку по вердикту круга — это
`AGENT_TOKEN` на его метках, то есть снятый упор против зацикливания, и
потому отдельное решение, а не правка по ходу.

Ключевое свойство: **разрешающие шаги делает только человек.** Агент вешает
`agent/needs-clarification`, `agent/in-progress`, `plan/proposed`,
`plan-review/N`, `fix/N`, `review/N` —
...
| Счётчик кругов ревью плана | метки `plan-review/1..2` на issue | да |
...
3. `gh pr view <номер> --comments` — план, тело обзора ревью, вердикты половин
   и номер попытки починки по меткам `fix/N`. Построчных замечаний — тех, что
   привязаны к файлу и строке, — в этом выводе нет: их печатает
   `bash scripts/review-comments.sh <номер PR>` (запись
...
| `plan-review/1`, `plan-review/2` | агент | Номер круга ревью плана |
...
Круги ревью плана считаются третьим, отдельным счётчиком — `plan-review/1..2`
на **issue**, а не на PR, которого к моменту плана ещё нет. Потолок тот же и
по той же причине (запись 0016), а счётчик отдельный потому, что предмет
другой: замечание к плану и замечание к диффу — разные поводы, и общий
счётчик их смешивал бы. Арифметику считает `scripts/plan-review-round.sh` —
вынесена из workflow ради того, чтобы потолок проверялся прогоном при
выключенном цикле.

Все три потолка выше — самый строгий вариант. Сколько кругов идёт у конкретной
задачи, зависит от её уровня риска: источник — политика `pipeline/route.json`,
маршрут уровня печатает `bash scripts/route.sh level <уровень>`. Поднять
потолок уровень не может ни в одну сторону — он только позволяет остановиться
раньше, — а `scripts/plan-review-round.sh` считает по самому строгому варианту,
потому что агентский цикл политику не читает (запись
[0020](decisions/0020-route-by-risk-level.md)).
...
План публикуется не прозой, а рендером машиночитаемой формы
(`scripts/plan-schema.json`), и до публикации проходит детерминированный
валидатор `scripts/plan.sh validate`. Не прошедший план публикуется вместе с
отчётом валидатора, но `plan/proposed` на нём не появляется: читать такой план
полезно, согласовывать — рано. Машиночитаемую форму в тот же комментарий
вкладывает сам рендер; зачем и каким блоком — [`rules/plan.md`](rules/plan.md),
раздел «Форма — машиночитаемая».

Между публикацией и апрувом стоит круг ревью плана — `step-review-correctness`
в режиме «предмет ревью — план» (`docs/rules/review-correctness.md`). Он
снимает с владельца сверку фактов и не снимает решения.

Стоит он не на каждой задаче. Есть ли этот круг, каков его потолок и сколько
кругов ревью PR пройдёт задача — функция её уровня риска. Источник правила
один: политика `pipeline/route.json`. Маршрут конкретного уровня печатает `bash
scripts/route.sh level <уровень>`, а `bash scripts/route.sh check` следит,
чтобы правило не разъехалось со своими потребителями — этим документом в том
числе. Апрув плана человеком в сокращения не входит ни на одном уровне.

**Режимы здесь больше не расходятся.** Локально круг запускает оркестратор, в
цикле — событие `issues: [labeled]` в том же `agent-review.yml`: два job,
`plan-round` и `plan-correctness`, стоят рядом с PR-половиной и не
пересекаются с ней ничем — гейты трёх её job требуют события `pull_request`
или `issue_comment` и на метке ложны.

Как это работает по шагам:

1. `agent-plan` публикует план и ставит `plan/proposed` — обе команды **под
   `AGENT_TOKEN`**, поимённым префиксом; сам шаг остаётся под штатным
   токеном. Штатный токен не годится для этих двух дважды: его события не
   создают запусков, а его комментарий приходит от бота, чей
   `authorAssociation` не равен `OWNER`, — плана в задаче не нашлось бы.
2. `plan-round` считает круг по меткам `plan-review/1..2`
   (`scripts/plan-review-round.sh`) и ставит метку **до** вызова модели: она
   считает круги, а не успехи. Два круга исчерпаны — комментарий и
   `agent/needs-human`.
3. `plan-correctness` **сам выбирает предмет ревью** — последний комментарий
   владельца, `jq` по `authorAssociation`, — и кладёт его текст в файл, а
   модель ревьюит этот файл. Нет такого комментария — падает шаг, а не
   модель возвращает вердикт об этом. Дальше читаются
   `docs/rules/review-correctness.md` и задача, вердикт публикуется
   комментарием. Ревью на безопасность у плана нет: его предмет — дифф,
   которого на момент плана не существует.

   Почему выборка в шаге, а не требование к модели: гейт job проверяет
   `author_association` **задачи**, а комментарий в публичной задаче вправе
   оставить кто угодно. Пока предмет ревью искала модель по фразе промпта,
   разграничение «план — не подделка по форме плана» держалось текстом,
   который правится свободно и ни одним сценарием не защищён.
4. При `changes_requested` одна команда возвращает задачу владельцу:
   `agent/needs-clarification` добавляется, `plan/proposed` и `agent/ready`
   снимаются. Снятие `agent/ready` — условие перезапуска: триаж отсекает
   задачу с этой меткой. При `approve` метки не трогаются вовсе —
   `plan/approved` остаётся человеческим.

Гейт плановой половины — пять конъюнктов, продублированных в обоих job:
выключатель цикла, событие `issues`, имя метки `plan/proposed`, авторство
задачи владельцем и отсутствие `agent/needs-human`. Первые четыре дословно
повторяют гейт `agent-plan.yml`, пятый добавлен потому, что `agent-review.yml`
обслуживает три события, а не одно.

Плата за третье событие — пустые запуски. Одна публикация плана даёт семь
запусков, и по существу работает один — плановая половина, ради неё метка и
ставится. Пустых шесть: четыре от метки (`agent-triage`, `agent-plan` и
`agent-implement` отсекаются на имени метки, а `status-sync` просыпается и не
переводит карточку никуда — `plan/proposed` в его таблице статусов нет) и два
от комментария (`agent-review` и `agent-fix`, оба на конъюнкте
`github.event.issue.pull_request != null`). Вызова модели в пропущенном
запуске нет, платы тоже: шесть строк в истории запусков — не поломка.
...
| Поведение валидатора плана | — | `scripts/plan.test.sh` |
| Поведение риск-скора | — | `scripts/risk-score.test.sh` |
| Маршрут по уровню риска ↔ его потребители | `scripts/route.sh check` | `scripts/route.test.sh` |
| Вход планового круга ревью | — | `scripts/plan-review-round.test.sh` |
| Чтение построчных замечаний ревью | — | `scripts/review-comments.test.sh` |

Харнесс валидатора плана здесь по той же причине, что и два предыдущих: сам
валидатор запускается при публикации плана, а не на push, — планов в
репозитории нет. Его сценариям нужна **история**, потому что три реальных плана
из истории задач проверяются против того дерева, для которого писались. Отсюда
`fetch-depth: 0` у checkout этой работы: на срезе в один коммит ни один пин не
разрешается, и харнесс краснеет в CI, оставаясь зелёным локально.

Харнесс планового круга — по той же причине, усиленной вдвое: сам счётчик
кругов запускается только job `plan-round` в `agent-review.yml`, а цикл
выключен, и живого прогона у плановой половины нет вовсе. Сценарии проверяют
две разные вещи: арифметику потолка — прогоном скрипта на наборах меток, а
провод — чтением текста `agent-review.yml` и `agent-plan.yml`. У каждой
проверки провода есть отрицательный сценарий на временной копии файла с
вырезанной строкой: проверка, зеленеющая на сломанном файле, проверяет
собственное существование. **Чего эти сценарии не ловят** — синтаксической
ошибки в YAML: они читают текст условия построчно, а `actionlint` в проекте
нет.

Харнесс риск-скора — по той же причине, что харнесс валидатора: сам
`scripts/risk-score.sh` из CI не запускается вовсе (ТЗ требует локального
запуска на всех фазах, и уровень риска ничего не блокирует), поэтому в его
шапке стоит маркер освобождения от обвязки. **Истории репозитория его
сценариям не нужно:** режимы `diff` и `history` проверяются на репозиториях,
собранных во временных каталогах, — сценарий, зависящий от того, что вчера
смерджили, краснел бы по чужой причине.

Маршрут по уровню риска стоит в этом списке двумя строками, и это первый
случай, когда в графе «чем» стоит не тире: сам маршрут никого не блокирует и
печатается подкомандой `level` руками, а из CI зовётся `bash scripts/route.sh
check` — сверка политики с её потребителями. Ту же работу для правил шагов
делает `rules-sync.sh`, но предмет у него другой: связи правил с промптами и
определениями, а не `pipeline/**`. Отрицательные сценарии харнесса идут на
временных политиках и временных копиях потребителей — сверка, зеленеющая на
политике с тремя одинаковыми маршрутами, проверяет собственное существование.
...
Харнесс чтения построчных замечаний стоит в графе «чем» с тире по причине,
названной в записи [0024](decisions/0024-line-comments-via-named-scenario.md):
сам `scripts/review-comments.sh` ходит в GitHub и запускается шагом починки, а
не CI. В CI идёт только харнесс — на подставном `gh`, без сети.
...
и приложенный к PR лишний trx-отчёт, добивающий сумму до снимка. Последнее —
обход известный, а не незнакомый приём, и закрыт он в гейте, а не в
инварианте: проверка 8 `scripts/guard.sh` считает нарушением `*.trx` и любой
файл под каталогом результатов в диффе PR — туда же отчёт покрытия, который
порог покрытия суммирует тем же способом. Метки-обхода у проверки нет (#90).
...
объявляет это маркером `# БЕЗ ОБВЯЗКИ:` с причиной в своей шапке. Причина
пишется рядом с маркером и там же читается — список освобождённых печатает сам
`wiring.sh`, поэтому здесь он не дублируется числом, которое устареет от
следующего скрипта. Освобождены сегодня: разовый ручной инструмент под токеном
записи (`bootstrap.sh`), файл, который источается, а не запускается
(`protected-paths.sh`), локальный инструмент, запускаемый руками по требованию
ТЗ (`risk-score.sh`), и скрипт, который вызывает хук `PreToolUse` из
определений `.claude/agents/` (`step-bash-allow.sh`).
...
- **Allow auto-merge** — включить. Это то, ради чего затевалась защита:
  ...
  Пункт не закрыт, и от этого зависит одна позиция маршрута из
  `pipeline/route.json`: мердж по уровням там описан заранее, но пока настройки
  нет, строка инертна в обе стороны — она не удерживает человека на высоком
  уровне и не раздаёт авто-мердж на низком и среднем. День включения меняет
  маршрут двух уровней сразу, поэтому включать его вместе с задачей, которая
  маршрут описывает, владелец отказался (запись
  [0020](decisions/0020-route-by-risk-level.md)).

  **Условие «чеки позеленели» вердикта ревью не содержит.** Апрувов требуется
  ноль (см. выше), а обе половины ревью PR публикуются комментариями и в
  required-чеки не входят вовсе: вердикт `changes_requested` мердж не остановит.
  Поэтому предусловие мерджа записано в самой политике — значениями позиции
  `merge` на низком и среднем уровнях, — а не отложено до дня включения
  настройки. `gh pr merge --auto` ставится после вердикта обеих половин, и это
  то, что придётся соблюдать процессом: настройки, которая это выражает, в
  GitHub при нулевом требовании апрувов нет.
...
Поэтому `AGENT_TOKEN` стоит ровно в шести местах, где событие обязано
разбудить следующий шаг:

| Где | Что делает | Кого будит |
|---|---|---|
| `agent-triage.yml`, шаг «Применение вердикта» | ставит `agent/ready` | `agent-plan` |
| `agent-plan.yml`, шаг «Публикация плана» | публикует план, ставит `plan/proposed` | плановый круг в `agent-review` |
| `agent-implement.yml`, шаг «Реализация» | пушит ветку, открывает PR | `CI (fast)` |
| `agent-review.yml`, шаг «Запрос починки» | пишет комментарий `/fix` | `agent-fix` |
| `agent-fix.yml`, шаг «Починка» | пушит правку | `CI (fast)` |
| `agent-fix.yml`, шаг «Ревью заново» | пишет комментарий `/review` | `agent-review` |

Две строки про комментарии — «Запрос починки» и «Ревью заново» — это те самые
плечи, которыми цикл замкнут комментариями (записи 0017 и 0016). Комментарий от
штатного токена не разбудил бы ни `agent-fix`, ни `agent-review`, и оба плеча
работали бы только тогда, когда команду пишет человек.

Строка про публикацию плана — про две причины, а не одну. Событие метки будит
круг, а автор комментария решает, найдёт ли круг предмет ревью: планом
считается последний комментарий владельца, и опубликованный штатным токеном
приходит от бота. Поэтому `AGENT_TOKEN` получают **обе команды** шага —
`gh issue comment` с планом и `gh issue edit --add-label 'plan/proposed'`, — а
не одна лишь метка. Получают поимённым префиксом: PAT лежит в окружении всего
шага и наследуется дочерними процессами, но `gh` без префикса берёт штатный
токен шага — сужен радиус автоматической аутентификации `gh`, а не окружения.

Под штатным токеном остаётся ветка «план не составлен» со своим комментарием и
обе `gh issue edit` с `agent/needs-clarification`: будить ими некого. Ветка
«валидатор не пропустил» своего комментария не имеет — план вместе с отчётом
валидатора публикует та же общая строка под PAT, и это осознанно: от штатного
токена такой комментарий пришёл бы от бота, а выборка предмета ревью
(последний комментарий владельца) разрешилась бы в предыдущий, устаревший
план.
...
- [x] `scripts/bootstrap.sh` прогнан: 14 агентских меток заведены,
      ...
      метками `agent/allow-*` снимается гейт целостности. **С тех пор в
      скрипт добавлены `plan-review/1` и `plan-review/2`** — в репозитории их
      ещё нет, пункт про повторный прогон ниже.
...
- [ ] Включить «Allow auto-merge», оставить только squash-мердж, включить
      автоудаление веток. Первое из трёх меняет маршрут по уровню риска:
      инертная сегодня позиция мерджа в `pipeline/route.json` начнёт работать в
      обе стороны — и заработает вместе со своим предусловием: авто-мердж
      ставится после вердикта обеих половин ревью, потому что required-чеком
      ревью не является и зелень чеков его не заменяет.
...
- [ ] Прогнать `scripts/bootstrap.sh` заново — ради меток `plan-review/1` и
      `plan-review/2`. Скрипт идемпотентен, повторный запуск безопасен. Не
      заведённая метка роняет первый же круг ревью плана на шаге «Отметка
      круга».
