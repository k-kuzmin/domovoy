...
1. `agent-plan` публикует план и ставит `plan/proposed` — **под
   `AGENT_TOKEN`**, весь шаг целиком. Штатный токен не годится дважды: его
   события не создают запусков, а его комментарий приходит от бота, чей
   `authorAssociation` не равен `OWNER`, — круг не нашёл бы плана.
2. `plan-round` считает круг по меткам `plan-review/1..2`
   ...
   `agent/needs-human`.
3. `plan-correctness` берёт предметом ревью последний комментарий владельца,
   читает `docs/rules/review-correctness.md` и публикует вердикт
...
Поэтому `AGENT_TOKEN` стоит ровно в четырёх местах, где событие обязано
...
Четвёртая строка — про две причины, а не одну. Событие метки будит круг, а
автор комментария решает, найдёт ли круг предмет ревью: план ищется среди
комментариев владельца, и опубликованный штатным токеном приходит от бота.
Поэтому на `AGENT_TOKEN` переведён **весь шаг**, а не одна команда с меткой.
