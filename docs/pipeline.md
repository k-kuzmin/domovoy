...
**Режимы здесь расходятся, и это названо, а не замалчивается.** Локально круг
запускает оркестратор. В агентском цикле его пока нет: `agent-review.yml`
запускается по событиям PR, события на `plan/proposed` там не предусмотрено, и
триггер — отдельная задача. Права для него уже есть, не хватает ровно события.
