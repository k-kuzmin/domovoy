...
Плата за третье событие — пустые запуски. Одна публикация плана даёт их
шесть: четыре от метки (`agent-triage`, `agent-plan`, `agent-implement` и
плановая половина — каждый отсекается на имени метки) и два от комментария
(`agent-review` и `agent-fix`, оба на конъюнкте
...
не одна лишь метка. Получают поимённым префиксом: остальному шагу PAT не
нужен, там работают `jq` над выводом модели и `scripts/plan.sh`, а ветки «план
не составлен» и «валидатор не пропустил» и по смыслу не должны никого будить.
