...
Разделы ниже описывают цикл в его включённом виде. При работе через CLI те же
пять шагов разыгрываются в одном разговоре, а точки вмешательства человека
остаются те же: апрув плана и мердж.
...
`Проверки обвязки` — работа про сам инструментарий: сверка правил шагов с
промптами (`scripts/rules-sync.sh`) и харнессы обеих shell-проверок,
`scripts/rules-sync.test.sh` и `scripts/guard.test.sh`. Имя шире прежнего
«Правила шагов и промпты» именно поэтому: харнесс гейта целостности к
правилам шагов отношения не имеет, но живёт здесь — гейту его условие
`pull_request` нужно, а харнессу нет, и на push в `main` он обязан идти.
